...
class RainwaveSong:
    ...
    def addedon(self):
        ...
        if 'song_addedon' not in self._raw_info:
            self._extend()
        ts = self._raw_info['song_addedon']
    ...
    def artists(self):
        ...
        if not hasattr(self, '_artists'):
            self._artists = []
            if 'artists' not in self._raw_info:
                self._extend()
            for raw_artist in self._raw_info['artists']:
                artist_id = raw_artist['artist_id']
    ...
    def available(self):
        ...
        not."""
        if 'song_available' not in self._raw_info:
            self._extend()
        return self._raw_info['song_available']
    ...
    def favourite(self):
        ...
        if 'song_favourite' not in self._raw_info:
            self._extend()
        return self._raw_info['song_favourite']
    ...
    def favourite(self, value):
        ...
        if 'fav_song_result' in d:
            if d['fav_song_result']['code'] == 1:
                self._raw_info['song_favourite'] = value
            else:
                raise Exception(d['fav_song_result']['text'])
        else:
    ...
    def id(self):
        """The ID of the song."""
        return self._raw_info['song_id']

    @property
    def lastplayed(self):
        """The UTC date and time the song last played (a
        :class:`datetime.datetime` object)."""
        if 'song_lastplayed' not in self._raw_info:
            self._extend()
        ts = self._raw_info['song_lastplayed']
        return datetime.datetime.utcfromtimestamp(ts)
    ...
    def rating(self):
        ...
        if 'song_rating_user' not in self._raw_info:
            self._extend()
        return self._raw_info['song_rating_user']

    @rating.setter
    def rating(self, value):
        d = self.album.channel.rate(self.id, value)
        if 'rate_result' in d:
            if d['rate_result']['code'] == 1:
                self._raw_info['song_rating_user'] = value
            else:
    ...
    def rating_avg(self):
        ...
        if 'song_rating_avg' not in self._raw_info:
            self._extend()
        return self._raw_info['song_rating_avg']
    ...
    def rating_channel_id(self):
        ...
        if 'song_rating_sid' not in self._raw_info:
            self._extend()
        return self._raw_info['song_rating_sid']

    @property
    def rating_count(self):
        """The total number of ratings given to the song by all listeners."""
        if 'song_rating_count' not in self._raw_info:
            self._extend()
        return self._raw_info['song_rating_count']
    ...
    def rating_id(self):
        ...
        if 'song_rating_id' not in self._raw_info:
            self._extend()
        return self._raw_info['song_rating_id']
    ...
    def releasetime(self):
        ...
        in the past."""
        if 'song_releasetime' not in self._raw_info:
            self._extend()
        ts = self._raw_info['song_releasetime']
        return datetime.datetime.utcfromtimestamp(ts)

    @property
    def secondslong(self):
        """The length of the song in seconds."""
        if 'song_secondslong' not in self._raw_info:
            self._extend()
        return self._raw_info['song_secondslong']
    ...
    def timesdefeated(self):
        ...
        if 'song_timesdefeated' not in self._raw_info:
            self._extend()
        return self._raw_info['song_timesdefeated']

    @property
    def timesplayed(self):
        """The number of times the song has played."""
        if 'song_timesplayed' not in self._raw_info:
            self._extend()
        return self._raw_info['song_timesplayed']

    @property
    def timeswon(self):
        """The number of times the song won an election."""
        if 'song_timeswon' not in self._raw_info:
            self._extend()
        return self._raw_info['song_timeswon']

    @property
    def title(self):
        """The title of the song."""
        if 'song_title' not in self._raw_info:
            self._extend()
        return self._raw_info['song_title']

    @property
    def totalrequests(self):
        """The total number of times the song has been requested by any
        listener."""
        if 'song_totalrequests' not in self._raw_info:
            self._extend()
        return self._raw_info['song_totalrequests']

    @property
    def totalvotes(self):
        """The total number of election votes the song has received."""
        if 'song_totalvotes' not in self._raw_info:
            self._extend()
        return self._raw_info['song_totalvotes']

    @property
    def url(self):
        """The URL of more information about the song."""
        if 'song_url' not in self._raw_info:
            self._extend()
        return self._raw_info['song_url']

    @property
    def urltext(self):
        """The link text that corresponds with :attr:`url`."""
        if 'song_urltext' not in self._raw_info:
            self._extend()
        return self._raw_info['song_urltext']
...
class RainwaveCandidate(RainwaveSong):
    ...
    def conflicts_with(self):
        ...
        if self.isconflict:
            name = self._raw_info['song_requestor']
